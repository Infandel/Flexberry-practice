    ...
    public class OrderBS : ICSSoft.STORMNET.Business.BusinessServer
    {
        ...
        private static DataObject[] changeOrderStatusPaid(IIS.Shop.Order UpdatedObject)
        {
            ...
            for (int i = 0; i < order.OrderItem.Count; i++)
            {
                ...
                // В случае последней итерации создаём накладную с соответствующим заказом.
                if (order.OrderItem.Count - i == 1)
                {
                    Invoice invoice = new Invoice { Order = order, CreateDate = System.DateTime.Now, Status = InvoiceStatus.New };
                    retObjs.Add(invoice);
                }
